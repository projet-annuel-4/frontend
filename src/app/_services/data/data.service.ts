...
export class DataService {
  ...
  sortFriends() {
    ...
    msgs.forEach((msg, k) => {
      ...
      if (friend.lastMsgAt < msg.createdAt) {
  ...
  getMessagesByChatId(chatId: String): Observable<UserMessage[]> {
    ...
    return this.httpClient
      .get(
        `${chat_service.CHAT}/${chatId}/messages/?user-email=${this.tokenStorage.getUser().email}`,
        this.httpOptions
      )
      .pipe(
  ...
  getFriends(): Observable<FriendProfile[]> {
    ...
    return this.httpClient
      .get<FriendProfile[]>(
        `${chat_service.CHAT}/?user-email=${this.tokenStorage.getUser().email}`,
        this.httpOptions
      )
      .pipe(
        map((friendProfiles: FriendProfile[]) => {
