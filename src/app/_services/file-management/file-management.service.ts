...
export class FileManagementService {
  ...
  uploadDirectoryFiles(directoryRequest: DirectoryRequest, files: FileResponse[]): Observable<any> {
    ...
    return this.http.post(
      `${file_service.BASE_URL}/${directoryRequest.id}`,
      bodyRequest,
      this.fileOptions
    )
  }
  ...
  downloadFile(file_id: number, type: string): Observable<FileResponse> {
    return this.http.get<FileResponse>(
      `${file_service.BASE_URL}/${file_id}/?type=${type}`,
      this.fileOptions
    )
  }
  ...
  downloadDirectoryFiles(directory_id: number, type: string): Observable<Directory> {
    return this.http.get<Directory>(
      `${file_service.BASE_URL}/directory/${directory_id}/?type=${type}`,
      this.fileOptions
    )
  }
  ...
  uploadImage(imageRequest: ImageRequest, file: File): Observable<string> {
    ...
    data.append('details', new Blob([jsonImageRequest], { type: 'application/json' }))
  ...
  deleteImage(image_id: number): Observable<FileResponse> {
    return this.http.delete<FileResponse>(
      `${file_service.BASE_URL}/image/${image_id}`,
      this.fileOptions
    )
  }
}
