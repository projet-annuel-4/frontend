...
export class ChatService {
  ...
  fetchFriends(): Observable<any> {
    return this.httpClient
      .get(
        `${environment.DOMAIN}/${environment.API_VERSION}/${environment.CHAT}/?user-email=${this.userEmail}`,
        this.httpOptions
      )
      .pipe(
        map((friends: FriendProfile[]) => {
  ...
  getAllUserMessages(): Observable<any> {
    return this.httpClient
      .post(
        `${environment.DOMAIN}/${environment.API_VERSION}/${environment.CHAT}/messages?user-email=${this.userEmail}`,
        Array.from(this.dataService.getAllFriend().keys()),
        this.httpOptions
      )
      .pipe(
        map((msgs: UserMessage[]) => {
  ...
  startConversation(email: String): Observable<any> {
    return this.httpClient
      .post(
        `${chat_service.CHAT}/?user-email=${this.userEmail}&friend-email=${email}`,
        '',
        this.httpOptions
      )
      .pipe(
  ...
  createMessageText(cid: string, content: string): Observable<UserMessage> {
    return this.httpClient
      .post(
        `${chat_service.CHAT}/${cid}/messages/text?content=${content}&user-email=${this.userEmail}`,
        {},
        this.httpOptions
      )
      .pipe(
  ...
  createMessageFile(cid: string, content: string, data: FormData): Observable<UserMessage> {
    return this.httpClient
      .post(
        `${chat_service.CHAT}/${cid}/messages/files?content=${content}&user-email=${this.userEmail}`,
        data,
        this.fileOptions
      )
      .pipe(
